...
@DADAMODELS.register_module()
class DADA_Augmenter(nn.Module):
    ...
    def forward(self, xyz):
        ...
        #############
        # probs_local, masking = self.predict_prob_layer(xyz, fps_idx)     # probs: [B, N, 3]   masking: [B, N, 2]
        # probs = self.predict_prob_layer(xyz, fps_idx)     # probs: [B, N, 3] 
        # probs = self.predict_prob_layer(xyz, fps_idx)     # [B, M, 3]
        probs_local, masking, offset = self.predict_prob_layer(xyz, fps_idx)     # [B, M, 3]
...
class SAComponent(nn.Module):
    ...
    def forward(self, x, a_index=None):
        ...
        offset = self.fuse_offset(offset).permute(0, 2, 1)       
