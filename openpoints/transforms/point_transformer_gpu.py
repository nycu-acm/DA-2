...
class RandomDropout(object):
    def __init__(self, dropout_ratio=[0.2, 0.5], dropout_application_ratio=0.2, **kwargs):
        """
        upright_axis: axis index among x,y,z, i.e. 2 for z
        """
        # self.dropout_ratio = dropout_ratio
        self.ratio_min, self.ratio_max = np.array(dropout_ratio).astype(torch.float32)
    ...
    def __call__(self, data):
        if random.random() < self.dropout_application_ratio:
            ...
            print(ratio)
            inds = torch.randperm(N)[:int(N * (1 - ratio))]
...
class PointcloudRandomInputDropout(object):
    def __init__(self, max_dropout_ratio=0.5, **kwargs):
    ...
    def __call__(self, data):
        ...
        drop_idx = np.where(np.random.random((data['pos'].shape[0])) <= dropout_ratio)[0]
        p = data['pos'][0].clone()
        if len(drop_idx) > 0:
            data['pos'][drop_idx] = p  # set to the first point
        return data
